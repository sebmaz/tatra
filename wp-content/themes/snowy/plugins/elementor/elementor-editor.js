...
jQuery( document ).ready(
	function() {
		...
		snowy_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="snowy_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.snowy_post_editor' );
						if ( link.length > 0 ) {
							snowy_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.snowy_post_editor', function(e) {
						snowy_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('snowy_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function snowy_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
