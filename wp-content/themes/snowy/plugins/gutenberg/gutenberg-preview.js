...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			snowy_create_observer( 'snowy-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				snowy_gutenberg_editor_init();
			} );
		} else {
		 	snowy_gutenberg_editor_init();
		}
	...
	function snowy_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'snowy_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'snowy_inited' ) ) {
	...
	// Init on page load
	function snowy_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = snowy_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + SNOWY_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + SNOWY_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in SNOWY_STORAGE['override_classes'] ) {
				$obj.addClass( SNOWY_STORAGE['override_classes'][i].replace( '%s', SNOWY_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( snowy_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && SNOWY_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'snowy_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			snowy_remove_observer( 'snowy-check-editor-styles-wrapper' );
			snowy_create_observer( 'snowy-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'snowy_inited' );
	}
} );
