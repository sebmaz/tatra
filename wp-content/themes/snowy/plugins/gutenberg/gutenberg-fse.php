...
if ( ! defined( 'SNOWY_FSE_TEMPLATE_PART_PT' ) ) define( 'SNOWY_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'SNOWY_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'SNOWY_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'SNOWY_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'SNOWY_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'SNOWY_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'SNOWY_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'SNOWY_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'SNOWY_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once snowy_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once snowy_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'snowy_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'snowy_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function snowy_gutenberg_fse_loader() {
		// 
		if ( snowy_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& snowy_is_on( snowy_get_theme_option( 'front_page_enabled', false ) )
			&& snowy_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'snowy_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function snowy_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'snowy_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function snowy_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'snowy_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function snowy_exists_gutenberg_fse() {
		return snowy_exists_gutenberg() && ( snowy_gutenberg_is_fse_wp_support() || snowy_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'snowy_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function snowy_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( snowy_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( snowy_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( SNOWY_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( SNOWY_THEME_DIR != SNOWY_CHILD_DIR && is_readable( SNOWY_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( SNOWY_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( SNOWY_THEME_DIR != SNOWY_CHILD_DIR && is_readable( SNOWY_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'snowy_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function snowy_gutenberg_is_fse_enabled() {
		return snowy_exists_gutenberg_fse() && snowy_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'snowy_gutenberg_fse_skin_updated' ) ) {
	add_action( 'snowy_action_theme_updated', 'snowy_gutenberg_fse_skin_updated' );
	add_action( 'snowy_action_skin_updated', 'snowy_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'snowy_action_skin_updated', 'snowy_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function snowy_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'snowy_action_theme_updated' && empty( $skin ) ) {
			$skin = snowy_skins_get_active_skin_name();
		}
		if ( snowy_skins_get_current_skin_name() == $skin ) {
			snowy_gutenberg_fse_copy_from_skin( $skin );
			snowy_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'snowy_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'snowy_action_skin_switched', 'snowy_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'snowy_action_skin_switched', 'snowy_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function snowy_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = snowy_prepare_path( SNOWY_THEME_DIR . ( snowy_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = snowy_prepare_path( SNOWY_THEME_DIR . ( snowy_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = snowy_prepare_path( SNOWY_THEME_DIR . 'theme.json' );
		$skin_templates_dir = snowy_prepare_path( SNOWY_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( snowy_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = snowy_prepare_path( SNOWY_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( snowy_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			snowy_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			snowy_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			snowy_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			snowy_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			snowy_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'snowy_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'snowy_action_skin_switched', 'snowy_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'snowy_action_save_options', 'snowy_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'snowy_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'snowy_action_skin_switched', 'snowy_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'snowy_action_save_options', 'snowy_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'snowy_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'snowy_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function snowy_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'snowy_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = snowy_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( SNOWY_THEME_DIR . snowy_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = snowy_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				snowy_fpc( snowy_prepare_path( SNOWY_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'snowy_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'snowy_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function snowy_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'snowy' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'snowy' ),
							"area"  => SNOWY_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'snowy' ),
							"area"  => SNOWY_FSE_TEMPLATE_PART_AREA_SIDEBAR,
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'snowy' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'snowy' ),
...
*/
		...
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-text',
				"name"   => esc_html__( 'Background and text color', 'snowy' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_dark'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-dark',
				"name"   => esc_html__( 'Background and dark color', 'snowy' )
			);
		}
		if ( ! empty( $scheme['bg_color'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['duotone'][] = array(
				...
				"slug"   => 'bg-and-link',
				"name"   => esc_html__( 'Background and link color', 'snowy' )
			);
		}
		...
		if ( ! empty( $scheme['text_hover'] ) && ! empty( $scheme['text_link'] ) ) {
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Vertical from link color to hover color', 'snowy' ),
			);
			$data['settings']['color']['gradients'][] = array(
				...
				"name"     => esc_html__( 'Diagonal from link color to hover color', 'snowy' ),
			);
		}
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'snowy_filter_gutenberg_fse_theme_json_data', $data );
	}
}
