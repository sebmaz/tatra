...
jQuery( document ).ready( function() {
	...
	function snowy_init_media_elements(cont) {
		var audio_selector = snowy_apply_filters( 'snowy_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
	...
	function snowy_tabs_ajax_content_loader(panel, page, oldPanel) {
		...
		jQuery.post(
			...
			if (rez.error !== '') {
				...
				// Append posts
				panel
					.find( '.snowy_tab_holder,.snowy_tab_content_remove' ).remove().end()
	...
	function snowy_resize_video(cont) {
		...
		if ( SNOWY_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'snowy_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
