...
if ( function_exists( 'snowy_elementor_is_preview' ) && snowy_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			snowy_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	snowy_new_main_query(
		array(
			'post_type'      => snowy_get_theme_option( 'post_type' ),
			'category'       => snowy_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'snowy_filter_get_template_part', snowy_blog_archive_get_template() ) );
}
