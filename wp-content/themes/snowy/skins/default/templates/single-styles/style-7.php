...
if ( apply_filters( 'snowy_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$snowy_with_featured_image = snowy_is_with_featured_image( $snowy_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $snowy_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $snowy_with_featured_image ) {
                echo ' with_featured_image' . ( false && snowy_get_theme_option( 'single_parallax' ) == 0 ? ' snowy-full-height' : '' );
            }
		?>">
			<?php
