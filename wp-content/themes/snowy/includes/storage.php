...
if ( ! function_exists( 'snowy_storage_get' ) ) {
	/**
	 ...
	 */
	function snowy_storage_get( $var_name, $default = '' ) {
		global $SNOWY_STORAGE;
		return isset( $SNOWY_STORAGE[ $var_name ] ) ? $SNOWY_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'snowy_storage_set' ) ) {
	/**
	 ...
	 */
	function snowy_storage_set( $var_name, $value ) {
		global $SNOWY_STORAGE;
		$SNOWY_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'snowy_storage_empty' ) ) {
	/**
	 ...
	 */
	function snowy_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $SNOWY_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $SNOWY_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $SNOWY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'snowy_storage_isset' ) ) {
	/**
	 ...
	 */
	function snowy_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $SNOWY_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $SNOWY_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $SNOWY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'snowy_storage_unset' ) ) {
	/**
	 ...
	 */
	function snowy_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $SNOWY_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $SNOWY_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $SNOWY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'snowy_storage_inc' ) ) {
	/**
	 ...
	 */
	function snowy_storage_inc( $var_name, $value = 1 ) {
		global $SNOWY_STORAGE;
		if ( empty( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = 0;
		}
		$SNOWY_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'snowy_storage_concat' ) ) {
	/**
	 ...
	 */
	function snowy_storage_concat( $var_name, $value ) {
		global $SNOWY_STORAGE;
		if ( empty( $SNOWY_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'snowy_storage_get_array' ) ) {
	/**
	 ...
	 */
	function snowy_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $SNOWY_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $SNOWY_STORAGE[ $var_name ][ $key ] ) ? $SNOWY_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'snowy_storage_set_array' ) ) {
	/**
	 ...
	 */
	function snowy_storage_set_array( $var_name, $key, $value ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$SNOWY_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'snowy_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function snowy_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $SNOWY_STORAGE[ $var_name ][ $key ] ) ) {
			$SNOWY_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$SNOWY_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$SNOWY_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'snowy_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function snowy_storage_merge_array( $var_name, $key, $value ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$SNOWY_STORAGE[ $var_name ] = array_merge( $SNOWY_STORAGE[ $var_name ], $value );
		} else {
			$SNOWY_STORAGE[ $var_name ][ $key ] = array_merge( $SNOWY_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'snowy_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function snowy_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			snowy_array_insert_after( $SNOWY_STORAGE[ $var_name ], $after, $key );
		} else {
			snowy_array_insert_after( $SNOWY_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'snowy_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function snowy_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			snowy_array_insert_before( $SNOWY_STORAGE[ $var_name ], $before, $key );
		} else {
			snowy_array_insert_before( $SNOWY_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'snowy_storage_push_array' ) ) {
	/**
	 ...
	 */
	function snowy_storage_push_array( $var_name, $key, $value ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $SNOWY_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $SNOWY_STORAGE[ $var_name ][ $key ] ) ) {
				$SNOWY_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $SNOWY_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'snowy_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function snowy_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $SNOWY_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $SNOWY_STORAGE[ $var_name ] ) && is_array( $SNOWY_STORAGE[ $var_name ] ) && count( $SNOWY_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $SNOWY_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $SNOWY_STORAGE[ $var_name ][ $key ] ) && is_array( $SNOWY_STORAGE[ $var_name ][ $key ] ) && count( $SNOWY_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $SNOWY_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'snowy_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function snowy_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SNOWY_STORAGE[ $var_name ][ $key ] ) ) {
			$SNOWY_STORAGE[ $var_name ][ $key ] = 0;
		}
		$SNOWY_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'snowy_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function snowy_storage_concat_array( $var_name, $key, $value ) {
		global $SNOWY_STORAGE;
		if ( ! isset( $SNOWY_STORAGE[ $var_name ] ) ) {
			$SNOWY_STORAGE[ $var_name ] = array();
		}
		if ( empty( $SNOWY_STORAGE[ $var_name ][ $key ] ) ) {
			$SNOWY_STORAGE[ $var_name ][ $key ] = '';
		}
		$SNOWY_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'snowy_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function snowy_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $SNOWY_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SNOWY_STORAGE[ $var_name ] ) ? $SNOWY_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $SNOWY_STORAGE[ $var_name ] ) ? $SNOWY_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'snowy_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function snowy_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $SNOWY_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $SNOWY_STORAGE[ $var_name ]->$prop ) ? $SNOWY_STORAGE[ $var_name ]->$prop : $default;
	}
}
