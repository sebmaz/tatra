<div class="front_page_section front_page_section_team<?php
	$snowy_scheme = snowy_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $snowy_scheme ) && ! snowy_is_inherit( $snowy_scheme ) ) {
		echo ' scheme_' . esc_attr( $snowy_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( snowy_get_theme_option( 'front_page_team_paddings' ) );
	if ( snowy_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$snowy_css      = '';
		$snowy_bg_image = snowy_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$snowy_anchor_icon = snowy_get_theme_option( 'front_page_team_anchor_icon' );
	$snowy_anchor_text = snowy_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( snowy_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$snowy_css      = '';
			$snowy_bg_mask  = snowy_get_theme_option( 'front_page_team_bg_mask' );
			$snowy_bg_color_type = snowy_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $snowy_bg_color_type ) {
				$snowy_bg_color = snowy_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$snowy_caption = snowy_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $snowy_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $snowy_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $snowy_caption, 'snowy_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$snowy_description = snowy_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $snowy_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $snowy_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $snowy_description ), 'snowy_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! snowy_exists_trx_addons() ) {
						...
						snowy_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
